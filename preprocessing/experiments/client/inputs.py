...
class Inputs:
    ...
    def _set_config_sheets(self):
        print("inputs: ",self.experiment.inputs)
    ...
    def _set_config_sheet(self, input_name, config_sheet):
        try:
            print("trying config_json_for: "+config_sheet)
            self._inputs.set_input(
                input_name, self.experiment.config_json_for(config_sheet)
            )
            # fetch same data as a usable datatable for preprocessing & validation
            self.rawData[config_sheet] = self.experiment.config_datatables_for(config_sheet)
            print(self.rawData[config_sheet])
