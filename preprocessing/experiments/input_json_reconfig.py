...
class InputJSONReconfig:
    def __init__(self, name, path, sheet_name, rawData):
        self.write_to_path = path / f"{name}_{sheet_name}.txt"
        self.dataRaw  = rawData[sheet_name] #pd.read_excel(path / file_name, sheet_name=sheet_name)
        self.data = rawData[sheet_name].to_json(   #pd.read_excel(path / file_name, sheet_name=sheet_name).to_json(
            orient="records"
        )
